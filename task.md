...
- `[x]` **Prioridade 2: Rentabilidade por OS (Custo vs Lucro)** ✅
  - `[x]` Gravar snapshot do `preco_custo` atual do produto na hora da alocação na OS.
  - `[x]` Criar modal gerencial no frontend ("Encerramento de OS") exibindo Margem de Lucro (Apenas perfil OWNER).
- `[x]` **Prioridade 3: Base Instalada, Grids e Visão 360º (Lazy Loading)** ✅
  - `[x]` Criar modelo de "Equipamento Base Instalada" atrelado ao cliente.
  - `[x]` Implementar gatilho de bloqueio (Lazy Loading): exigir atualização do aparelho para Base Instalada ao movimentar OSs legadas.
  - `[x]` Atualizar a sessão "Clientes" para uma Data Table interativa com filtros dinâmicos (estilo Protheus).
  - `[x]` Adicionar "Motor de Recorrência": flag vermelha se >2 OS em 90 dias.
