...
export default function ClientManager({ clients, userRole, isOffline, onRefresh, limit, onLimitChange, searchTerm, onSearchChange, totalItems }: ClientManagerProps) {
  ...
  const filteredClients = clients.filter(c => {
    ...
    return matchClient(c, searchTerm);
  });
        ...
        <div className="flex flex-col sm:flex-row items-center gap-2 flex-1">
          <div className="relative shadow-sm rounded-xl flex-1 w-full">
            <div className="relative w-full">
              ...
              <input
                type="text"
                placeholder="Pesquisar por nome, telefone, CPF/CNPJ, endereço ou equipamento..."
                value={searchTerm}
                onChange={(e) => onSearchChange(e.target.value)}
                className="w-full pl-10 pr-10 py-2 bg-white border border-slate-200 rounded-lg text-sm focus:outline-none focus:border-indigo-500 focus:ring-2 focus:ring-indigo-500/20 transition-all font-medium"
              />
              {searchTerm && (
                <button
                  type="button"
